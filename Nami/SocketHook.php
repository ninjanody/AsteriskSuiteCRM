...
class SocketHook {
    function load_ws(){
        ...
        global $current_user;
        if(!is_null($current_user->sip_c) && $current_user->sip_c != "" && $_GET['module'] != "ModuleBuilder" && $_GET['module'] != "Emails" && $_GET['sugar_body_only'] != 1 && empty($_REQUEST['to_pdf']) && empty($_REQUEST['sugar_body_only'] && $_REQUEST['massupdate'] == false && $_REQUEST['action'] != "MassUpdate")) {
            echo <<<SCRIPT
<script>
jQuery.fn.extend({
...
if($('#websocket').length > 0)
 ...
 socket = new WebSocket("ws:89.184.66.252:8081/ws"); 
    ...
    socket.onopen = function(){
       $('body').append('<input type"hidden" id="websocket"/>');
         ...
         var html_linked_lead = '<div id="call_lead_info"><a id="linked_lead" href="/index.php?action=ajaxui#ajaxUILoc=index.php%3Fmodule%3DLeads%26action%3DEditView%26return_module%3DLeads%26return_action%3DDetailView">Создать лида</a></div>';
                if(card_obj.linked_id != null)
                {
                    ...
                    linked_leads_arr.forEach(function(item, l, arr){
                        linked_lead = window.location.protocol + "//" + window.location.hostname + "/index.php?action=DetailView&module="+card_obj.linked_type+"&record="+item;
               ...
               redirect_button = ""; 
               if( isInboundCall(card_obj.call) )
                {
    ...
    socket.onmessage = function(msg){
      ...
      if(message_arr['event'] == "Hangup")
       ...
       localStorage.setItem(message_arr['call_id'], JSON.stringify(locStorageObj));
         var html_linked_lead = '<div id="call_lead_info"><a id="linked_lead" href="/index.php?action=ajaxui#ajaxUILoc=index.php%3Fmodule%3DLeads%26action%3DEditView%26return_module%3DLeads%26return_action%3DDetailView">Создать лида</a></div>';
                if(message_arr['linked_id'] != null)
                {
                    ...
                    linked_leads_arr.forEach(function(item, l, arr){
                        linked_lead = window.location.protocol + "//" + window.location.hostname + "/index.php?action=DetailView&module="+message_arr['linked_type']+"&record="+item;
                        html_linked_lead += '<div id="call_lead_info"><a id="linked_lead" href="'+linked_lead+'">'+linked_name_leads_arr[l].replace('null', '')+'</a></div>';
                    });
                }
       var bottom_pos_for_new = 0;
       ...
       redirect_button = "";
       if( isInboundCall(message_arr['call']) )
         {
    ...
    function isInboundCall(call_type){
        return (call_type == "Входящий") ? true : false;
    };
